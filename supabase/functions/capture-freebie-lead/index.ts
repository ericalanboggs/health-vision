...
// Friendly display names for the freebie slugs we offer.
const FREEBIE_NAMES: Record<string, string> = {
  'summit-weekly-reflection': 'Summit Weekly Reflection',
}
...
serve(async (req) => {
  ...
  try {
    ...
    const freebieName = FREEBIE_NAMES[freebieSlug] || 'Summit'
    ...
    const downloadUrl = `${FRONTEND_URL}/freebies/${freebieSlug}.zip`
    const logoUrl = `${FRONTEND_URL}/summit-logo.png`
    const emailResult = await sendEmail({
      to: email,
      subject: `Your ${freebieName} skill — download inside`,
      html: buildDownloadHtml(downloadUrl, logoUrl, freebieName),
    })
